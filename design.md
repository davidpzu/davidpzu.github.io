...
| 4 | Sanofi Connect - Referral functionality | Referral functionality designed to grow platform audience and enable potential reward system | `work/sconnect/` |
...
- **The space to the right of the list is deliberately empty.** David asked whether to fill it — exam results, Korean books, a photo from a concert — and decided against, after the options were weighed. Recorded so it is not re-opened:
  - **The block sits in the credentials half of the passage**, between Education & Experience and the toolkit. An image here has to read as a credential, not as biography — and the personal register is already carried, well, by §3.5's three chapter collages, which include concerts and travel. **A concert photo belongs in `me-now`, not here.**
  - **A certificate was rejected outright.** It restates a level the text already gives, reads as a badge, and carries a name and certificate number.
  - **It is not free.** The index is at 793.7 KB raw / 717.2 KB transferred against §6's 800. A ~580px AVIF is 60–90 KB, which puts raw over and the wire figure at the line. It would need a recovery first — `me-now.avif` is 224 KB at quality 85 and would give back 60–80 KB re-encoded at 65, the setting the thumbnails already use.
  - **Anything added later has to work on `--void`.** The chapter images are cut-outs made for light grounds. A photo here is shot dark and bleeds into the ground, or it sits on a `--paper` tile like the tool icons — which puts a large light rectangle into the darkest part of the page. Decide that against the actual photo, not in advance.
  - **The emptiness is already load-bearing.** `.lang-list`'s 32rem cap exists so the hairlines stop cleanly rather than trailing across it, and the hero has a larger empty right half that reads as composition. This is the same move.
...
- **A lead line under the headline**, before the two columns: *"If you want to know more about me send me an email and I'll get back as soon as possible."* Body face, not mono — it is a sentence, where everything else in this footer is a label or a field. **Capped at `48ch`, not `--measure`, and set `text-wrap: balance`.** At 62ch the sentence very nearly fits one line and leaves *"possible."* alone on a second at the widest viewports; `balance` makes the browser pick the wrap point that evens the lines instead, which is what a hand-placed `<br>` was doing badly — it fixed one width and broke the rest. Verified at 1440 / 1024 / 768 (two even lines) and 375 (three), no orphan at any of them. Unsupported browsers wrap normally, which is the behaviour without the rule.
...
- **SACEM's two Final Outcome images landed 2026-08-28**, both 2280 × 1280 (0.19% off exact 16:9, which is invisible, and identical to each other, which is what matters for two stacked figures). AVIF q85, 92.0 and 104.4 KB, taking the page to 506.9 KB against §6's 800. **Export size for the remaining six: 2280 × 1283**, 2× the 1140px `--shell` box the figure occupies — the same 2280 width as the hero, so every case-study export shares one number.
- **Their captions were rewritten to match the pictures, and lost v1's "Feature N —" numbering with it.** The second one had described a notification centre that is not in the image. The page's other caption — *"Screenshots of the old messaging system"* — was already plain and unnumbered, and §3.3 removed numbering from the work cards on the same reasoning: it is an index nobody uses. The three unfinished case studies still carry v1's numbered captions beside their placeholders; rewrite each with its images.
- **The other three case studies still have their six placeholder boxes**, on the §3.5 rule: a placeholder is a box, the real image is not.
...
- **Every image David owed has landed.** The three chapter images, all four card thumbnails and all four case study heroes are in and wired. What is still outstanding is the **Final Outcome screens** — SACEM's two landed on 2026-08-28, leaving six across the other three pages, still placeholders carrying v1's captions. The rule stands for those: build the fixed-ratio slot, and **do not source, generate, or substitute images.**
